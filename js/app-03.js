function home(){
  const featured=state.providers.slice(0,4);
  return `<section>
    ...
    <div class="editorial"><div class="eyebrow">HOY MATCH</div><h2>Problem statt Branchenbuch.</h2><p>Du beschreibst Aufgabe, Ort, Zeitpunkt und Sprache. HOY filtert die Betriebe, die fachlich und regional wirklich passen.</p><button data-start-request>Anfrage starten</button></div>
    <div class="source">Öffentliche Anbieterprofile sind keine automatische HOY-Partnerschaft. Quellenstatus, bestätigte Einsatzgebiete und Live-Verfügbarkeit werden getrennt behandelt. Regionale Bilder sind als Umgebungsbilder gekennzeichnet.</div>
  </section>`;
}

function filtered(){
  const q=state.query.trim().toLowerCase();
  return state.providers.filter(p=>(state.filter==='all'||(p.cats||[]).includes(state.filter))&&(!q||[p.name,p.area,p.summary,serviceNames(p)].join(' ').toLowerCase().includes(q)));
}
function discover(){
  const list=filtered();
  return `<section><div class="head"><div class="head-top">${brand()}<button class="round" data-start-request>${icons.plus}</button></div><h1>Services.</h1><p>Lokale Anbieter rund um La Manga, Cabo de Palos und Umgebung – mit sichtbar getrenntem Quellen- und Vertrauensstatus.</p><div class="searchline"><input id="q" placeholder="Anbieter, Leistung oder Ort …" value="${esc(state.query)}"><button data-search>${icons.compass}</button></div></div>${cloudStrip()}<div class="filterline"><button class="${state.filter==='all'?'active':''}" data-filter="all">Alle</button>${categories.map(c=>`<button class="${state.filter===c.id?'active':''}" data-filter="${c.id}">${c.label}</button>`).join('')}</div><div class="list">${list.map(listCard).join('')||'<div class="empty"><h2>Nichts gefunden.</h2><p>Ändere Suche oder Filter.</p></div>'}</div></section>`;
}
function saved(){
