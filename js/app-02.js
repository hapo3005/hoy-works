...
function verification(p){return p.verification==='source_checked'?'<span class="pill good">✓ Quelle geprüft</span>':p.verification==='directory_only'?'<span class="pill warn">! Verzeichnisfund</span>':'<span class="pill warn">Daten prüfen</span>'}
...
function cloudStrip(){const online=state.backend==='online';const error=state.backend==='error';return `<div class="trust-strip"><span>${online?`${state.providers.length} Anbieter · live aus HOY Works Supabase`:error?`${state.providers.length} geprüfte Fallback-Profile`:'HOY Works verbindet sich …'}</span><b class="cloud-state ${online?'online':error?'error':''}">${online?'CLOUD LIVE':error?'FALLBACK':'VERBINDET'}</b></div>`}

function card(p){return `<article class="card" data-open="${esc(p.id)}"><div class="card-art">${mediaMarkup(p)}<button class="heart ${state.favorites.has(p.id)?'active':''}" data-fav="${esc(p.id)}" aria-label="Merken">${icons.heart}</button></div><div class="card-body"><h3>${esc(p.name)}</h3><div class="meta">${esc(serviceNames(p))}<br>${esc(p.area||'Einsatzgebiet wird geprüft')}</div><div class="card-foot">${verification(p)}${(p.languages||[]).slice(0,2).map(l=>`<span class="pill">${esc(l)}</span>`).join('')}</div></div></article>`}
function listCard(p){return `<article class="list-card" data-open="${esc(p.id)}"><div class="list-art">${mediaMarkup(p)}</div><div><h3>${esc(p.name)}</h3><p>${esc(p.area||'Einsatzgebiet wird geprüft')}</p><div class="service">${esc(serviceNames(p))}</div></div><button class="heart2" data-fav="${esc(p.id)}">${icons.heart}</button></article>`}
