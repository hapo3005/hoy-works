...
for(const token of ['NEEDS_CONFIRMATION','business_confirmed','community_confirmed','disclosureRequired','organicRank','safetyStatus']){
...
for(const token of ['MUST / PREFER / IGNORE','Commercial integrity','Privacy','PARITY_CODE_COMPLETE']){
  if(!doc.includes(token))throw new Error(`Parity document missing: ${token}`);
}
console.log(JSON.stringify({ok:true,parity:'PARITY_CODE_COMPLETE',requiredFiles:required.length},null,2));
